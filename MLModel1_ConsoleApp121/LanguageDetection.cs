...
namespace MLModel1_ConsoleApp121
{
    ...
    {
        public static async Task LanguageDetectionRun()
        {
            ...
            foreach (var (lang, text) in LanguageDetectionData.ShortSamples)
            {
            ...
            foreach (var (lang, text) in LanguageDetectionData.LongSamples)
            {
                var doc = new Document(text);
                fastTextLanguageDetector.Process(doc);
            ...
            var allPredictions = fastTextLanguageDetector.Predict(new Document(LanguageDetectionData.LongSamples[Language.Spanish]));
